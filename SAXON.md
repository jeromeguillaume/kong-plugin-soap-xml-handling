...
- Adapt the version of the initContainer, Plugins or saxon (example: `kong-saxon-local-lib:1.0.5-1.4.0-12.5-beta.2.2`) in the following file:
...
docker run -d --name kong-gateway-soap-xml-handling \
...
jeromeguillaume/kong-soap-xml:3.10.0.0-1.4.0-12.5-beta.2.2
```
- Kubernetes:
  ...
  - Set in `values.yaml` the `image.repository` to `jeromeguillaume/kong-soap-xml:3.10.0.0-1.4.0-12.5-beta.2.2`. See a complete `values.yaml` example for Konnect: [values-4-Konnect.yaml](kong/saxon/kubernetes/values-4-Konnect.yaml)
...
deployment:
  initContainers:
  - name: kongsaxon
    image: jeromeguillaume/kong-soap-xml-initcontainer:1.0.5-1.4.0-12.5-beta.2.2
