...
- Adapt the version of the initContainer, Plugins or saxon (exemple: `kong-saxon-local-lib:1.0.5-1.2.5-12.5`) in the following file:
...
docker run -d --name kong-gateway-soap-xml-handling \
...
jeromeguillaume/kong-saxon:3.9.0.1-1.2.5-12.5
```
- Kubernetes:
  ...
  - Set in `values.yaml` the `image.repository` to `jeromeguillaume/kong-saxon:3.9.0.1-1.2.5-12.5`. See a complete `values.yaml` example for Konnect: [values-4-Konnect.yaml](kong/saxon/kubernetes/values-4-Konnect.yaml)
...
deployment:
  initContainers:
  - name: kongsaxon
    image: jeromeguillaume/kong-saxon-initcontainer:1.0.5-1.2.5-12.5
