...
- The `Saxon - Kong` integration requires 2 Shared Objects that are extracted and built with the `make` command:
  1) `libsaxon-hec-12.5.0.so`: C++ shared object extracted from `libsaxon-HEC-linux-<arch>-v12.5.0.zip`
  2) `libsaxon-4-kong.so`: C shared object compiled from `kong-adapter.cpp`
- Prerequisite
```sh
cd saxon
```
- Build and Extract the `saxon` libraries for Docker test. The libaries are built in `./saxon/so/<arch>` depending of the architecture:
  - `arm64`
  ```sh
...
## Run Kong with Saxon
1) Run `Saxon` in Docker and the official image: `kong/kong-gateway`
...
2) Run `Saxon` in Docker with the customized image: `jeromeguillaume/kong-saxon`
```sh
docker run -d --name kong-gateway-soap-xml-handling \
...
...
3) Run `Saxon` in Kubernetes with an `initContainer` image: `jeromeguillaume/kong-saxon-initcontainer`
