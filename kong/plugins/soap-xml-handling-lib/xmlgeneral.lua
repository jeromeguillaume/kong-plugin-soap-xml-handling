...
xmlgeneral.schemaTypeSOAP_All         = 1
...
xmlgeneral.WSDL                       = "WSDL"        -- WSDL type
xmlgeneral.XSD                        = "XSD"         -- XSD type
...
xmlgeneral.xmlSoapPtrCache            = "soap-xml-pc-" 
...
xmlgeneral.maxRecursionDepthImport    = 50
...
function xmlgeneral.detectContentType (contentType)
  ...
  if not lowerContentType then
    ...
    rc = xmlgeneral.schemaTypeSOAP1_1
  -- SOAP 1.1
  elseif string.find(lowerContentType, "^%s*text/xml")   then
    rc = xmlgeneral.schemaTypeSOAP1_1
  -- SOAP 1.2
...
function xmlgeneral.initializeContextualDataExternalEntities (plugin_conf)
  ...
  kong.ctx.shared.xmlSoapExternalEntity.xsdSoap12SchemaInclude  = plugin_conf.xsdSoap12SchemaInclude
  kong.ctx.shared.xmlSoapExternalEntity.filePathPrefix          = plugin_conf.filePathPrefix
  ...
  for k, prefetchConf_entry in pairs (prefetchConf_entries) do
    ...
    if xsdHashKey then
      ...
      if not errMessage then
        ...
        kong.log.debug("asyncPrefetch_Schema_Validation_callback: **Success** | PluginType:", conf.pluginType, "| count: ", count, "| duration=", xsdHashKey.duration)
      else
      ...
      kong.log.debug("asyncPrefetch_Schema_Validation_callback - PluginType:", conf.pluginType, " Last status: ", tostring(xsdHashKey.prefetchStatus), " Last message: ", (errMessage or "'N/A'"))
    
    -- Else the XSD 'hashKey' is not found in the 'entityLoader.hashKeys'
    else
      rc = false
      errMessage = "PluginType:"..conf.pluginType.." Unable to find XSD HashKey '" .. (prefetchConf_entry.xsdHashKey or "nil") .. "' in the 'entityLoader.hashKeys'"
...
function xmlgeneral.pluginConfigure_XSD_Validation_Prefetch (config, pluginType, wsdl_xsd, child, queueName, queue_conf)
  ...
  if kong.xmlSoapAsync.entityLoader.hashKeys[xsdHashKey].prefetchStatus == xmlgeneral.prefetchStatusInit then
    ...
    conf.cacheTTL = config.ExternalEntityLoader_CacheTTL
...
--  libxml+libxslt+saxon: 
--    => Initialize a Cache Table for Compiling and Parsing WSDL/XSD, XSLT
--    => Delete the Cache Table if the plugin is deleted
--
...
---------------------------------------------------------------------------------------------
function xmlgeneral.pluginConfigure (configs, pluginType)
  local saxon = false
  local countHashKeys = 0
  local countPointersCache = 0
  ...
  if configs then
    ...
    kong.log.debug("pluginConfigure, Begin #entityLoader.hashKeys=" , tostring(countHashKeys))      
    
    -- Purge the Pointers cache table for plugins using 'Saxon':
    ...
    --  Other Pointers cache tables are marked (as 'To Be Deleted') but, based on the content 'configs' sent by the CP, the flag will be set to 'false'
    for keyPluginId, cachePlugin in next, kong.xmlSoapSaxonPtrCache.plugins do
      
      if cachePlugin.pluginType == pluginType then        
        -- If during the previous call of 'configure' phase, the cache table has been marked as to be deleted
        if cachePlugin.toBeDeleted == true then
          if cachePlugin.XSLTs then
            ...
            if cachePlugin.xsltLibrary == 'saxon' and 
              cachePlugin.XSLTs[xmlgeneral.xsltBeforeXSD] and              
            ...
            -- If there is a current Contex
            if cachePlugin.xsltLibrary == 'saxon' and 
              cachePlugin.XSLTs[xmlgeneral.xsltAfterXSD] and               
          ...
          kong.xmlSoapSaxonPtrCache.plugins[keyPluginId] = nil -- Delete the whole cache of the plugin
          countPointersCache = countPointersCache + 1
        else
          -- First consider that the plugin should be deleted and
    ...
    kong.log.debug("pluginConfigure for Saxon, Deleted #Pointers Cache tables=", countPointersCache)

    -- Parse all instances of the plugin sent by the Control Plane for:
    --  => 'libxml2' and 'libxslt' libraries:
    --        Invalidate the Pointers cache table stored in 'kong.cache' 
    --  => 'Saxon' library:
    --        Creating the Pointers cache table
    --          OR
    --        Forcing a refresh of the table
    for keyConfig, config in ipairs(configs) do
      local plugin_id = config.__plugin_id
      
      -- 'libxml2' and 'libxslt' libraries: Invalidate the Pointers cache table
      kong.cache:invalidate_local(xmlgeneral.xmlSoapPtrCache..plugin_id)
      ...
      if config.xsltLibrary == 'saxon' then
        ...
        if not kong.xmlSoapSaxonPtrCache.plugins[plugin_id] then
          kong.xmlSoapSaxonPtrCache.plugins[plugin_id] = {}
          local pluginConf = kong.xmlSoapSaxonPtrCache.plugins[plugin_id]
          ...
          --    => so it avoids that pending requests crash due to old pointers (that aren't available anymore)
          local pluginConf = kong.xmlSoapSaxonPtrCache.plugins[plugin_id]
    ...
    local log_level = kong.configuration.log_level
    -- DEBUG only
    if log_level == "debug" then 
      -- DEBUG only: Get the number of 'entityLoader.hashKeys' (related to Async External Entities URL)    
      countHashKeys = 0
      for k, v in next, kong.xmlSoapAsync.entityLoader.hashKeys do
        countHashKeys = countHashKeys + 1
      end
      kong.log.debug("pluginConfigure: End #entityLoader.hashKeys=", tostring(countHashKeys))

      -- DEBUG only: Get the number of Pointers Cache table and the numbers that will be deleted on next Configure phase
      countPointersCache = 0
      local countPointersCacheTBD = 0
      for k,v in next, kong.xmlSoapSaxonPtrCache.plugins do
        if v.pluginType == pluginType then
          countPointersCache = countPointersCache + 1
          if v.toBeDeleted == true then
            kong.log.debug("pluginConfigure for Saxon, pluginId=", k, " mark as deleted in the cache")
            countPointersCacheTBD = countPointersCacheTBD + 1
          end
        end
      end
      kong.log.debug("pluginConfigure for Saxon, #Pointers_Cache_tables=", countPointersCache, " #Marked_as_deleted=", countPointersCacheTBD)
    end
  end
end
...
function xmlgeneral.xmlDumpToFile (fileName, xml_ptr, xml_src, xml_declaration, remove_empty_namespace)
  ...
  local file, errFile = io.open(fileName, "w")
  if file then
    file:write(xml_dump)
    file:close()
    kong.log.debug("XML dump is saved in the file: ", fileName)
  else
    errMessage = "Unable to save the XML dump into a file. Error: " .. (errFile or "nil")
    kong.log.err(errMessage)
  end
end

---------------------------------------------------------------------------
-- Dump into a String the XML transformed by the XSL Transformation
---------------------------------------------------------------------------
function xmlgeneral.xmlDump (xml_ptr, xml_src, xml_declaration, remove_empty_namespace)
  local xml_dump = ''
  local errDump
  local errMessage

  if xml_ptr ~= ffi.NULL and xml_ptr ~= nil then
    -- Dump into a String the canonized image of the XML transformed by XSLT
    xml_dump, errDump = libxml2ex.xmlC14NDocSaveTo (xml_ptr, nil)
    
    if errDump == 0 then
      -- If needed, append the xml declaration
      -- Example: <?xml version="1.0" encoding="utf-8"?>
      xml_dump =  (xml_declaration or '') .. xml_dump
      -- If needed, remove empty Namespace (example: xmlns="") added by XSLT transformation 
      if remove_empty_namespace then
        xml_dump = xml_dump:gsub(' xmlns=""', '')
      end
    else
      errMessage = "Unable to dump the canonized image of the XML document"
    end
  else
    xml_dump = (xml_src or '')
  end
  
  return xml_dump, errMessage
end

----------------------------------------------------------------------------------------
-- Prepare an XML declaration (which starts by '<?')
...
function xmlgeneral.XSLTransform_libsaxon(pluginType, pluginId, pluginConf, xsltBeforeAfterXSD, XMLptrToTransform, XMLtoTransform, XSLT)
  ...
  if not (  kong.xmlSoapSaxonPtrCache.plugins[pluginId] and 
            kong.xmlSoapSaxonPtrCache.plugins[pluginId].XSLTs and
            kong.xmlSoapSaxonPtrCache.plugins[pluginId].XSLTs[xsltBeforeAfterXSD]) then
    return nil, xmlgeneral.invalidPtrsCacheTable, xmlgeneral.soapFaultCodeServer
  end

  -- Get the XSLT Cache table  
  local cacheXslt = kong.xmlSoapSaxonPtrCache.plugins[pluginId].XSLTs[xsltBeforeAfterXSD]
  ...
  if not errMessage then
    ...
    --   OR
    -- If the XSLT is to be refreshed (due to a change of the plugin configuration or the TTL is reached)
    if (not cacheXslt.xsltPtr and not cacheXslt.xsltErrMsg) 
        or 
        cacheXslt.started + cacheTTL < ngx.now() then
      ...
      if cacheXslt.xsltPtr ~= ffi.NULL then
        kong.log.debug ("XSLT transformation, caching: Delete the current compiled XSLT from cache and replace it with the new one")
        libsaxon4kong.deleteContext (cacheXslt.xsltPtr)
      end
      cacheXslt.xsltPtr     = context
      cacheXslt.xsltErrMsg  = errMessage
      cacheXslt.started     = ngx.now()
...
function xmlgeneral.XSLTransform_libxslt(pluginType, pluginId, pluginConf, xsltBeforeAfterXSD, XMLptrToTransform, XMLtoTransform, XSLT)
  ...
  if err then
    return nil, nil, xmlgeneral.errorGettingPtrsCacheTable .. err, xmlgeneral.soapFaultCodeServer
  -- If the Pointers cache table is not initialized, return an error
  elseif not (  ptrsCacheTable and 
                ptrsCacheTable.XSLTs and
                ptrsCacheTable.XSLTs[xsltBeforeAfterXSD] ) then
    return nil, nil, xmlgeneral.invalidPtrsCacheTable, xmlgeneral.soapFaultCodeServer
  end
  
  -- Get the XSLT Cache table
  local cacheXslt = ptrsCacheTable.XSLTs[xsltBeforeAfterXSD]
  ...
  if errMessage == nil then
    ...
    elseif style == ffi.NULL then
      ...
      if xml_doc == nil then
        ...
        kong.log.debug("XSLT transformation, the XML to transform is not already parsed in memory")
        ...
        kong.log.debug("XSLT transformation, the XML to transform is already parsed in memory")
      end
    end
  else
    errMessage = xmlgeneral.invalidXSLT .. ". " .. errMessage
  ...
  if errMessage == nil then
    -- Transform the XML doc with XSLT transformation
    xml_transformed_ptr = libxslt.xsltApplyStylesheet(style, xml_doc, xsltParams)
    
    if xml_transformed_ptr ~= ffi.NULL then
      -- If needed we get the xml declaration
      -- Example: <?xml version="1.0" encoding="utf-8"?>
      xml_declaration = xmlgeneral.XSLT_Format_XMLDeclaration (
                                          style.version,
                                          style.encoding,
  ...
  return xml_transformed_ptr, xml_declaration, errMessage, soapFaultCode
...
function xmlgeneral.XSLTransform(pluginType, pluginId, pluginConf, xsltBeforeAfterXSD, XMLptrToTransform, XMLtoTransform, XSLT)
  ...
  elseif xsltLibrary == 'saxon' then
    ...
    errMessage = "XSLTransform: unknown library " .. (xsltLibrary or "nil")
    kong.log.err(errMessage)
  end

  -- If we have to remove empty Namespace (example: xmlns="") added by XSLT Transformation and not compatible with the WSDL/XSD validation
  if not errMessage and xsltRemoveEmptyNameSpace then
    if xsltLibrary == 'libxslt' then
      xml_transformed_dump, errMessage = xmlgeneral.xmlDump (xmlPtrTransformed, nil, xml_declaration, xsltRemoveEmptyNameSpace)
      -- Set to nil the pointer of the transformed XML document as we have now the XML transformed in a String format
      -- On next step the XML document will be re-parsed in memory (with 'xmlReadMemory')
      xmlPtrTransformed = nil
    elseif xsltLibrary == 'saxon' then
  ...
  return xmlPtrTransformed, xml_transformed_dump, xml_declaration, errMessage, soapFaultCode
end
...
function xmlgeneral.addSchemaLocation(xml_doc, cuurentNode)
  ...
  ------------------------------------------------------------------------------------------------
  -- Firstly loop on all <schema> entries for adding 'schemaLocation=' to <import> if not defined
  ------------------------------------------------------------------------------------------------
  kong.log.debug ("addSchemaLocation, get list of <import> without 'schemaLocation'")  
  while schemaNode ~= ffi.NULL do
    ...
    if tonumber(schemaNode.type) == ffi.C.XML_ELEMENT_NODE and 
      ...
      index = index + 1
      kong.log.debug ("addSchemaLocation, schema #", index)
      ...
      while childNode ~= ffi.NULL do
        ...
        if tonumber(childNode.type) == ffi.C.XML_ELEMENT_NODE and 
          ...
          if namespaceAttrPtr ~= ffi.NULL and schemaLocationAttrPtr == ffi.NULL then
            ...
            kong.log.debug ("addSchemaLocation, schema #", index, " add schemaLocation='", ffi.string(namespaceAttrPtr), "' to <import>")
  ...
  -----------------------------------------------------------------------------------------
  kong.log.debug ("addSchemaLocation, get XSD definition of targetNamespace related to the list of <import> without 'schemaLocation'")
  while schemaNode ~= ffi.NULL and schemaLocations do
    ...
    if tonumber(schemaNode.type) == ffi.C.XML_ELEMENT_NODE and 
      ...
      if ffi_targetNS ~= ffi.NULL then
        ...
        kong.log.debug ("addSchemaLocation, schema #", index, " targetNamespace='", targetNS, "'")
        ...
        if schemaLocations[targetNS] == "" then 
          ...
          xsdSchema = xmlgeneral.addNamespaces(xsdSchema, xml_doc, schemaNode)
...
---------------------------------------
-- Initialize the Pointers Cache table
---------------------------------------
function xmlgeneral.initPointersCacheTable(pluginType)
  local pointersCacheTable = {}
  pointersCacheTable.pluginType = pluginType  
  pointersCacheTable.WSDLs = {}
  pointersCacheTable.WSDLs[xmlgeneral.schemaTypeSOAP1_1] = {}
  pointersCacheTable.WSDLs[xmlgeneral.schemaTypeSOAP1_2] = {}
  pointersCacheTable.WSDLs[xmlgeneral.schemaTypeAPI ] = {}
  pointersCacheTable.WSDLs[xmlgeneral.schemaTypeSOAP1_1].XSDs = {}
  pointersCacheTable.WSDLs[xmlgeneral.schemaTypeSOAP1_2].XSDs = {}
...
------------------------------------------------------------------------------------------------------------
function xmlgeneral.loadXMLwithRecursiveImport(xmlOperation, importsAlreadyProcessed, depthImportsProcessed, filePathPrefix, input_API_definition, verbose)
  local errMessage              = nil
  local parentNodeName          = nil
  local parentChildNodeName     = nil
  local parentChildNameProp     = nil
  local parentWsdlNodeFound     = false
  local parentNodeFound         = false
  ...
  -- If the maximum recursion depth for the import/include is reached, we return an error
  if depthImportsProcessed == xmlgeneral.maxRecursionDepthImport then    
    errMessage = "Maximum recursion depth reached for the import/include of the WSDL/XSD definitions"
    kong.log.debug("loadXMLwithRecursiveImport, ", errMessage)
    return nil, errMessage, importsAlreadyProcessed, depthImportsProcessed
  end
  ...
  if (xmlOperation == xmlgeneral.WSDL and not parentWsdlNodeFound ) or
    ...
    errMessage = "The root node of the imported definition is not 'wsdl:definitions' nor 'wsdl:description' for WSDL and not 'xsd:schema' for XSD"
    return parent_xml_doc, errMessage, importsAlreadyProcessed, depthImportsProcessed
  else
    ...
    if tonumber(parent_currentNode.type) == ffi.C.XML_ELEMENT_NODE and parent_currentNode.name ~= ffi.NULL then      
      ...
      if  (parentNodeName == "import"   and xmlOperation == xmlgeneral.WSDL) or
        ...
        if locationPtr ~= ffi.NULL then
          ...
          -- Check that the imported document is not already processed (to avoid an infinite loop)
          if not importsAlreadyProcessed then
            importsAlreadyProcessed = {}
          end
          if importsAlreadyProcessed[locationStr] then
            kong.log.debug ("loadXMLwithRecursiveImport, the imported definition '", locationStr, "' is already processed. Do nothing and avoid an infinite loop")
            imported_xml_doc = ffi.NULL
          else
            importsAlreadyProcessed[locationStr] = {}
            kong.log.debug ("loadXMLwithRecursiveImport, Found <", parentNodeName, "> with location='", locationStr, "' in Parent document")
            depthImportsProcessed = depthImportsProcessed + 1  
            -- Do a recursive call to load the imported document and its dependencies
            imported_xml_doc, errMessage, importsAlreadyProcessed, depthImportsProcessed = xmlgeneral.loadXMLwithRecursiveImport(xmlOperation, importsAlreadyProcessed, depthImportsProcessed, filePathPrefix, locationStr, verbose)
            depthImportsProcessed = depthImportsProcessed - 1
          end                  
        end
      end
        ...
        if (xmlOperation == xmlgeneral.WSDL and not importedWsdlNodeFound ) or
          ...
          while importedCurrentNode ~= ffi.NULL and not errMessage do
            ...
            if tonumber(importedCurrentNode.type) == ffi.C.XML_ELEMENT_NODE and importedCurrentNode.name ~= ffi.NULL then
              -- Here we can have:
              -- Example #1: for <wsdl:types>                     => importedNodeName="types"   and importedNameProp=nil
              -- Example #2: for <wsdl:message name="AddSoapIn">  => importedNodeName="message" and importedNameProp="AddSoapIn"
              importedNodeName = ffi.string(importedCurrentNode.name)
              importedNameProp = libxml2.xmlGetProp(importedCurrentNode, "name")
            end
            
            -------------------------------------------------------------
            -- Look for an existing Imported node in the Parent document
            -------------------------------------------------------------
            parent_currentChildNode = libxml2.xmlFirstElementChild(parentXmlNodePtrRoot)
            parentNodeFound     = false
            parentChildNameProp = nil
            while parent_currentChildNode ~= ffi.NULL do
              parentChildNodeName = nil
              parentChildNameProp = nil
            ...
            -- If the parent document already contains the Imported node (<wsdl:types>, <wsdl:message>, etc.)            
            -- we add the childs of the Imported node (of the imported document) into the corresponding node of the parent document            
            if parentNodeFound == true then
              
              importedChildCurrentNode = libxml2.xmlFirstElementChild(importedCurrentNode)
              -- If there is a child node (of child node in the imported document), for instance: <xsd:schema>, <wsdl:operation>
              if importedChildCurrentNode ~= ffi.NULL then
                -- xmlDocCopyNodeList: copies the node and the siblings nodes including the child Node
                local copied_list = libxml2ex.xmlDocCopyNodeList (parent_xml_doc, importedChildCurrentNode)
                if copied_list ~= ffi.NULL then
                  libxml2ex.xmlAddChildList(parent_currentChildNode, copied_list)                
                else
                  errMessage = "Unable to copy the imported " .. xmlOperation .. " definition"
                end
              else
                kong.log.debug("loadXMLwithRecursiveImport, the imported node <", importedNodeName, "> has no child node to add in the parent document")
              end
            -- Else the parent document doesn't contain the Imported node (<wsdl:types>, <wsdl:message>, etc.)
            -- we add the Imported node (of the imported document) into the Parent document just below the root node of 
            -- the Parent document, it could be <wsdl:definitions>, <wsdl:description>, <xsd:schema>
            else
              -- xmlDocCopyNodeList: copies the node including the child Node (but does NOT copy the siblings nodes)
              local copied_list = libxml2ex.xmlDocCopyNode	(importedCurrentNode, parent_xml_doc, 1)
              if copied_list ~= ffi.NULL then
  ...
  if not errMessage then
    --if kong.configuration.log_level == "debug" then
    --  local fileName
    --  -- If the API content is a raw definition
    --  if i then
    --    fileName = "raw-"..string.format("%02d", depthImportsProcessed)..".wsdl"
    --  -- Else the API content is defined in a file
    --  else
    --    fileName = input_API_definition
    --  end
    --  -- Dump in a file the complete WSDL document with all the imported definitions (for debug purpose)
    --  libxml2ex.xmlDumpToFile ("/kong-plugin/temp/dump-"..fileName, parent_xml_doc)
    --end
...
function xmlgeneral.XMLValidateWithWSDL (pluginType, pluginId, pluginConf, child, XMLptrToValidate, XMLtoValidate, WSDL, prefetch)
  ...
  if prefetch or async then
    ...
    --    Here we have:
    --        prefetch=true
    --        pluginId=nil <- unable to call kong.cache:get(..pluginId..)
    --  OR 
    ...
    local ptrsCacheTable, err = kong.cache:get(xmlgeneral.xmlSoapPtrCache..pluginId, { ttl = cacheTTL }, xmlgeneral.initPointersCacheTable, pluginType)
    if err then
      return xmlToValidate_doc, xmlgeneral.errorGettingPtrsCacheTable .. err, xmlgeneral.soapFaultCodeServer
    -- If the Pointers cache table is not initialized, return an error
    elseif not (  ptrsCacheTable and 
                  ptrsCacheTable.WSDLs and 
                  ptrsCacheTable.WSDLs[child] and
                  ptrsCacheTable.WSDLs[child].XSDs) then
      return xmlToValidate_doc, xmlgeneral.invalidPtrsCacheTable, xmlgeneral.soapFaultCodeServer
    end
    cacheWSDL = ptrsCacheTable.WSDLs[child]
    
    local cacheOk = true
    -- If there is no XSD definitions it means that the WSDL is not compiled yet
    if not next(cacheWSDL.XSDs) then
      kong.log.debug ("WSDL Validation, 'configure' phase has been triggered or caching TTL (", (cacheTTL or 'nil')," s) is reached, so re-compile the WSDL")
    else
      -- Check that all XSD pointers are correctly compiled in the cache
    ...
    -- If the XSDs attached to the WSDL are not correctly compiled
    if not cacheOk then
      ptrsCacheTable.WSDLs[child] = {}
      cacheWSDL = ptrsCacheTable.WSDLs[child]
      cacheWSDL.XSDs = {}      
    end
  end
  ...
  if cacheWSDL.xmlWsdlPtr == nil or cacheWSDL.xmlWsdlPtr == ffi.NULL then
    ...
    if recursiveWsdlImport or recursiveXsdImport then
      ...
      if recursiveWsdlImport then
        xml_doc, errMessage = xmlgeneral.loadXMLwithRecursiveImport(xmlgeneral.WSDL, nil, 0, filePathPrefix, WSDL, nil, verbose)
  ...
  --   -> Asynchronous: Get all XSDs and compile/parse them for caching
  -------------------------------------------------------------------------------------------------
  if firstCall then
    ...
    if typesNodeFound then
      ...
      if forceSchemaLocation then
        ...
        if schemaLocations and next(schemaLocations) then
          ...
          if not kong.ctx.shared.xmlSoapExternalEntity.xsdApiSchemaInclude  then
            kong.ctx.shared.xmlSoapExternalEntity.xsdApiSchemaInclude = {}          
          end
...
function xmlgeneral.XMLValidateWithXSD (pluginType, pluginId, pluginConf, childInput, indexXSD, XMLptrToValidate, XMLtoValidate, XSDSchemaInput, XSDSchemaInput2, prefetch)
  ...
  -- If it's not a Prefetch we have to load the XML to be validated against the XSD schema
  if not prefetch then
    ...
    if not XMLptrToValidate then
      kong.log.debug("XSD validation, the XML to validate is not already parsed in memory")
      xmlToValidate_doc, errMessage = libxml2ex.xmlReadMemory(XMLtoValidate, false, filePathPrefix, nil, nil, default_parse_options, verbose, false)
    else
      kong.log.debug("XSD validation, the XML to validate is already parsed in memory")
      xmlToValidate_doc = XMLptrToValidate
    end
    ...
    -- For SOAP part: Get the SOAP version (1.1 or 1.2)
    elseif childInput == xmlgeneral.schemaTypeSOAP_All then
      -- Get Root Element, which is <soap:Envelope>
      local xmlNodePtrRoot = libxml2.xmlDocGetRootElement(xmlToValidate_doc);
      if xmlNodePtrRoot ~= ffi.NULL then
        if kong.configuration.log_level == "debug" then
          kong.log.debug ("XSD validation ", schemaType, " part: '", libxml2ex.xmlNodeDump(xmlToValidate_doc, xmlNodePtrRoot, 1, 1), "'")
        end
        
        -- Get the SOAP version (1.1 or 1.2) from the XML message
        raw_namespaces = libxml2.xmlGetNsList(xmlToValidate_doc, xmlNodePtrRoot)
        i = 0
        while not child  and
          raw_namespaces and raw_namespaces[i] ~= ffi.NULL and raw_namespaces[i].href ~= ffi.NULL do
          -- SOAP 1.1 NameSpace
          ...
          elseif ffi.string(raw_namespaces[i].href) == xmlgeneral.schemaSOAP1_2 then
            child = xmlgeneral.schemaTypeSOAP1_2
            XSDSchema = XSDSchemaInput2
          end
          i = i + 1
        end
        if child then
  ...
  if not errMessage then
    ...
    if prefetch or async then
      ...
      local ptrsCacheTable, err = kong.cache:get(xmlgeneral.xmlSoapPtrCache..pluginId, { ttl = cacheTTL }, xmlgeneral.initPointersCacheTable, pluginType)
      if err then
        return xmlToValidate_doc, xmlgeneral.errorGettingPtrsCacheTable .. err, false, xmlgeneral.soapFaultCodeServer
      -- If the Pointers cache table is not correctly initialized, return an error
      elseif not (  ptrsCacheTable and 
                    ptrsCacheTable.WSDLs and
                    ptrsCacheTable.WSDLs[child] and
                    ptrsCacheTable.WSDLs[child].XSDs) then
        return xmlToValidate_doc, xmlgeneral.invalidPtrsCacheTable, false, xmlgeneral.soapFaultCodeServer
      -- If 'configure' phase has been triggered or TTL is reached => let's create the XSD cache table
      elseif not ptrsCacheTable.WSDLs[child].XSDs[indexXSD] then
        ptrsCacheTable.WSDLs[child].XSDs[indexXSD] = {}
        kong.log.debug ("XSD Validation, 'configure' phase has been triggered or caching TTL (", (cacheTTL or 'nil'), " s) is reached, so re-compile the XSD")
      else
        -- Get XSD pointers from the cache table
        cacheXSD = ptrsCacheTable.WSDLs[child].XSDs[indexXSD]
  ...
  if prefetch then
    return xmlToValidate_doc, errMessage, XMLXSDMatching, soapFaultCode
  end
  ...
  if not errMessage then
    ...
    if childInput == xmlgeneral.schemaTypeAPI then
      -- Example:
      ...
      if xmlNodePtrChildWS ~= ffi.NULL and tonumber(xmlNodePtrChildWS.type) == ffi.C.XML_ELEMENT_NODE then
        ...
        if kong.configuration.log_level == "debug" then
          kong.log.debug ("XSD validation ", schemaType, " part: '", libxml2ex.xmlNodeDump(xmlToValidate_doc, xmlNodePtrChildWS, 1, 1), "'")
        end
    ...
    ----------------------    
      ...
      is_valid, errMessage = libxml2ex.xmlSchemaValidateDoc (validation_context, xmlToValidate_doc, verbose)              
    end    
  end
...
function xmlgeneral.getSOAPActionFromWSDL (pluginId, pluginConf, request_OperationName, xmlnsSOAPEnvelope_hRef)
  ...
  if async then
    ...
    elseif not (  ptrsCacheTable and 
                  ptrsCacheTable.WSDLs and 
                  ptrsCacheTable.WSDLs[xmlgeneral.schemaTypeAPI] and
                  ptrsCacheTable.soapAction) then
      errMessage = xmlgeneral.invalidPtrsCacheTable
    else
      xmlWSDL_doc     = ptrsCacheTable.WSDLs[xmlgeneral.schemaTypeAPI].xmlWsdlPtr
      cacheSoapAction = ptrsCacheTable.soapAction
    end    
  end
  
  -- If the XML Tree document of WSDL is not in the cache
  if xmlWSDL_doc == nil or xmlWSDL_doc == ffi.NULL or errMessage then
    -- This code should never happen because if the pointer is null it will be raised before by 'xmlgeneral.XMLValidateWithWSDL'
    return wsdlSoapAction_Value, wsdlRequired_Value, (errMessage or "Unable to get the XML Tree document. The WSDL is not in the cache")
  end
  ...
  if not errMessage and wsdlDefinitions_type == xmlgeneral.WSDL1_1 then
    ...
    if object ~= ffi.NULL then
      ...
      if object.nodesetval ~= ffi.NULL and object.nodesetval.nodeNr ~= 0 then
        ...
        kong.log.debug("getSOAPActionFromWSDL: the optional 'xpathReqRequired' attribute is not found", (errXPathSoapAction or "nil"))
      end
    else
...
function xmlgeneral.validateSOAPAction_Header (pluginId, pluginConf, SOAPptrToValidate, SOAPRequest)
  ...
  local xmlRequest_doc        = SOAPptrToValidate
  ...
  if SOAPAction_Header == xmlgeneral.SOAPAction_Header_No then
    ...
    return xmlRequest_doc, nil, soapFaultCode
  end
  ...
  if not errMessage then
    if not xmlRequest_doc then
      kong.log.debug("Validate 'SOAPAction' Header - the XML to validate is not already parsed in memory")
      ...
      kong.log.debug("Validate 'SOAPAction' Header - the XML to validate is already parsed in memory")
    end
  end
  ...
  if not errMessage then
    ...
    while raw_namespaces and raw_namespaces[i] ~= ffi.NULL and raw_namespaces[i].href ~= ffi.NULL do
      if  ffi.string(raw_namespaces[i].href) == xmlgeneral.schemaSOAP1_1 then
        nsSOAP_11_12_found = xmlgeneral.schemaTypeSOAP1_1
      elseif ffi.string(raw_namespaces[i].href) == xmlgeneral.schemaSOAP1_2 then
        nsSOAP_11_12_found = xmlgeneral.schemaTypeSOAP1_2
      end
  ...
  if not errMessage then
    if  nsSOAP_11_12_found == xmlgeneral.schemaTypeSOAP1_1 and SOAPAction12_Header_Value then
      errMessage = "Found a SOAP 1.1 envelope and an 'action' field in the 'Content-Type' header linked with for SOAP 1.2"
    elseif nsSOAP_11_12_found == xmlgeneral.schemaTypeSOAP1_2 and SOAPAction11_Header_Value then
  ...
  return xmlRequest_doc, errMessage, soapFaultCode
end
...
function xmlgeneral.RouteByXPath (pluginId, pluginConf, XMLptrToSearch, XMLtoSearch)
  ...
  -- v1.4.4 Disable the 'xmlNewParserCtxt' that doesn't provide extra information and that forces 
  -- the usage of 'xmlCtxtReadMemory' instead of 'xmlReadMemory' => it's better for performance
    ...
    for i, RouteXPathTarget in pairs(RouteXPathTargets) do
      ...
      if object ~= ffi.NULL then
        ...
        if object.nodesetval ~= ffi.NULL and object.nodesetval.nodeNr ~= 0 then        
            ...
            kong.log.debug("libxml2.xmlNodeGetContent: ", (nodeContent or "nil"))
  ...
  if rcXpath > 0 then
    kong.log.debug ("RouteByXPath: END: successfully changed route, ",
                    "with XPath='", RouteXPathTargets[rcXpath].XPath,
                    "' and XPathCondition='", RouteXPathTargets[rcXpath].XPathCondition, "'")
  else
    kong.log.debug ("RouteByXPath: END: not having the proper XPath expression or the right condition")
  end
  return document_ptr, rcXpath, errMessage, xmlgeneral.soapFaultCodeServer
end
